//! Data pipeline: text loading, tokenization, batching.
//!
//! Supports text files or JSONL (one text field per line). Tokenizer loaded from
//! tokenizer.json (e.g. GPT-2 BPE). Batches are (batch_size, seq_len) token IDs;
//! labels for next-token prediction are input_ids shifted by one.
//!
//! Use `StreamingBatchIter` to train on large datasets without loading all into memory.
...
/// Dataset over text: yields tokenized sequences from files or directory.
pub struct TextDataset {
    /// Path to a file or directory of text/JSONL files.
    path: PathBuf,
    /// Tokenizer (BPE etc.). Must match config.vocab_size.
    tokenizer: Tokenizer,
    /// Maximum sequence length (chunk size).
    seq_len: usize,
    /// Accumulated token IDs from all read text (flattened).
    token_ids: Vec<u32>,
}
...
impl TextDataset {
    ...
    /// Load and tokenize all text from path (file or directory). Call before iterating.
    ...
    fn load_file(&mut self, path: &Path) -> anyhow::Result<()> {
        ...
        for line in reader.lines() {
            ...
            // If JSONL: support "text", or "input"+"output" (e.g. Alibaba Superior-Reasoning-SFT)
            let text: String = if line.starts_with('{') {
    ...
    /// Total number of tokens (after load).
    pub fn num_tokens(&self) -> usize {
        self.token_ids.len()
    }

    /// Number of full sequences of length seq_len.
    ...
    pub fn batches(&self, batch_size: usize) -> impl Iterator<Item = (Vec<u32>, Vec<u32>)> + '_ {
        ...
        let step = seq_len; // no overlap for simplicity
...
/// Streaming batch iterator: reads files line-by-line and yields batches without
/// loading the full dataset into memory. Use for large datasets (e.g. 25GB+).
...
impl StreamingBatchIter {
    /// Create a streaming iterator over path (file or directory of .jsonl/.txt).
    pub fn new(
        path: &Path,
        tokenizer_path: &Path,
        seq_len: usize,
    ...
    /// Pull more tokens from the current file (or advance to next file) into buffer.
    ...
    /// Yield the next batch, or None if no more data.
...
/// Convert a batch of (input_ids, labels) to Candle tensors on the given device.
